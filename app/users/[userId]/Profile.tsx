        ...
        <div className='flex items-center gap-2 mt-4'>
          <div className='flex -space-x-2'>
            {user.followeds.map((followed) => (
              <Avatar size='sm' className='border-2 border-background' key={followed.follower.id}>
                {followed.follower.image ? (
                  <AvatarImage src={followed.follower.image} alt={followed.follower.username} />
                ) : null}
                <AvatarFallback>{followed.follower.username.slice(0, 2).toUpperCase()}</AvatarFallback>
              </Avatar>
            ))}
          </div>
